...
__sbit bFlag_is_key_pressed = vFlag_1 : 2;
...
//     ENI(); // GIE = 1;
// }
#define CONST_P_KEY_FILTER (2)
void gpioKeyScan(void)
{
    static unsigned char counter;
    // P_KEY_INPUT;
    if (bFlag_is_key_pressed)
    {
        if (P_KEY)
        {
            if (++counter > CONST_P_KEY_FILTER)
            {
                counter = 0;
                P_KEY_IPS_SET;
                bFlag_is_key_pressed = 0;
                // P_KEY_IPS_SET;
                P_KEY_IPS_INPUT;
            }
        }
...
void is_chg_or_dischg(void)
{
    ...
    if (bFlag_charger_on) //! 在充电状态下
    {
        ...
        if ((reg_ip53xs) && (0 == (IP_DATA[1] & BIT3))) // ! 同充同放 要显示充电的ui
        {
            // io_uart_tx(0xDD);
            // io_uart_tx(0xDD);
    ...
    else // 在放电状态下
    {
        ...
        else
        {
            ...
            {
                displayTimer = CONST_TIMER_DISP_XSEC;
                // io_uart_tx(0xCC);
                // io_uart_tx(0xCC);
            }
            else
...
void app_display_all(void) // 500ms ONE  time
{
    ...
    else if (bFlag_charger_on) // bFlag_charger_nbFlag_vin_plug_in
    {
        ...
        {
            displayHundred = SEG_E; //% DISP_ALL_OFF;
            if (bFlag_qc_ok)
            {
    ...
    while (TRUE)
    {
        ...
        if (bFlag_gSysTick_Change)
        {
            ...
            if (bFlag_slice_16ms)
            {
                ...
                bFlag_slice_16ms = 0;
                timer_slice_16ms++;
                if (0 == bFlag_charger_on)
                {
                    gpioKeyScan(); //! 充电不需要按键功能
                }
                else
                {
                    P_KEY_IPS_INPUT;
